    ...
    scope.launch {
      ...
      val device =
          Device.newBuilder()
              ...
              .build()
      val deviceUiModel = DeviceUiModel(device, testDevice.isOnline, testDevice.mode.toLong())
      // Add the device to the repository.
      devicesRepository.addDevice(deviceUiModel.device)
      devicesStateRepository.addDeviceState(
          deviceUiModel.device.deviceId, deviceUiModel.isOnline, deviceUiModel.mode)
    }
  }

  private val TEST_DEVICES =
      listOf(
          TestDevice("1", "11", "Light", false, 0),
          TestDevice("2", "22", "Light", false, 1),
          TestDevice("3", "33", "Light", true, 0),
          TestDevice("4", "44", "Light", true, 1),
          TestDevice("5", "55", "Outlet", false, 0),
          TestDevice("6", "66", "Outlet", false, 1),
          TestDevice("7", "77", "Outlet", true, 0),
          TestDevice("8", "88", "Outlet", true, 1),
          TestDevice("9", "99", "Fan", false, 0),
          TestDevice("10", "1010", "Fan", false, 1),
          TestDevice("11", "1111", "Fan", true, 0),
          TestDevice("12", "1212", "Fan", true, 1),
      )
