...
class DevicesAdapter(
    ...
    private val switchClickListener: (View, DeviceUiModel, Int) -> Unit
  ...
  override fun onBindViewHolder(holder: DeviceViewHolder, position: Int) {
    ...
    if (repoItem != null) {
      ...
      val onOffOption = holder.itemView.findViewById<MaterialButtonToggleGroup>(R.id.onoff_option)

      val btnOff = holder.itemView.findViewById<MaterialButton>(R.id.button_off)
      val btnFree = holder.itemView.findViewById<MaterialButton>(R.id.button_free)
      val btnBusy = holder.itemView.findViewById<MaterialButton>(R.id.button_busy)
      btnOff.setOnClickListener { switchClickListener(holder.itemView, repoItem, 0) }
      btnFree.setOnClickListener { switchClickListener(holder.itemView, repoItem, 1) }
      btnBusy.setOnClickListener { switchClickListener(holder.itemView, repoItem, 2) }
    }
  }
