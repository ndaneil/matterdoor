...
constructor(
  ...
  private fun processDevices(
    ...
    devices.devicesList.forEach { device ->
      ...
      if (state == null) {
        ...
        devicesUiModel.add(DeviceUiModel(device, isOnline = false, 0))
      } else {
        Timber.d("    deviceId setting its own value for state")

        devicesUiModel.add(DeviceUiModel(device, state.online, state.mode))
      }
    }
  ...
  fun commissionDeviceSucceeded(activityResult: ActivityResult, deviceName: String) {
    ...
    viewModelScope.launch {
      ...
      try {
        ...
        devicesStateRepository.addDeviceState(deviceId, isOnline = true, mode = 0)
        _commissionDeviceStatus.postValue(
  ...
  fun updateDeviceStateOn(deviceUiModel: DeviceUiModel, which: Long) {
    Timber.d("updateDeviceStateOn: Device [${deviceUiModel}]  which: [${which}]")
    viewModelScope.launch {
      Timber.d("Handling real device")
      clustersHelper.setLevelDeviceStateOnOffCluster(deviceUiModel.device.deviceId, which.toInt(), 1)
      devicesStateRepository.updateDeviceState(deviceUiModel.device.deviceId, true, which)
        //TODO add other handlings
    }
  }
              ...
              override fun onReport(nodeState: NodeState) {
                ...
                viewModelScope.launch {
                  devicesStateRepository.updateDeviceState(
                      device.deviceId, isOnline = true, mode = level.toLong())
                }
              }
            }
    ...
    viewModelScope.launch {
      while (devicesPeriodicPingEnabled) {
        ...
        devicesList.forEach { device ->
          ...
          var level = clustersHelper.getDeviceStateLevelCluster(device.deviceId, 1)
          ...
          Timber.d("runDevicesPeriodicPing deviceId [${device.deviceId}] [${isOnline}] [${level}]")
          // TODO: only need to do it if state has changed
          devicesStateRepository.updateDeviceState(
              device.deviceId, isOnline = isOnline, mode = level.toLong())
        }
