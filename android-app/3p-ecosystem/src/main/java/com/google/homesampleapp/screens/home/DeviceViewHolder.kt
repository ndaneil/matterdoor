...
class DeviceViewHolder(private val binding: DeviceViewItemBinding) :
  ...
  private val matterDoorDeviceIcon = getResourceId("ic_launcher_foreground")
  ...
  fun bind(deviceUiModel: DeviceUiModel) {
    ...
    val iconResourceId =
        when (deviceUiModel.device.deviceType) {
          ...
          Device.DeviceType.TYPE_MATTERDOOR -> matterDoorDeviceIcon
          else -> unknownDeviceIcon
        }
    val shapeDrawable =
        if (deviceUiModel.isOnline && deviceUiModel.mode != 0L) shapeOnDrawable else shapeOffDrawable
    binding.zeicon.setImageResource(iconResourceId)
    binding.name.text = deviceUiModel.device.name
    binding.zestate.text = stateDisplayString(deviceUiModel.isOnline, deviceUiModel.mode != 0L)
    binding.rootLayout.background = shapeDrawable
    if (ON_OFF_SWITCH_DISABLED_WHEN_DEVICE_OFFLINE) {
      binding.onoffOption.isEnabled = deviceUiModel.isOnline
    } else {
