...
class DeviceFragment : Fragment() {
  ...
  private fun setupUiElements() {
    ...
    binding.onoffOption.addOnButtonCheckedListener { group, checkedId, isChecked ->
      if(!isChecked) return@addOnButtonCheckedListener
      var which = 0
      if (binding.buttonFree.id == checkedId) which = 1
      else if (binding.buttonBusy.id == checkedId) which = 2
      viewModel.updateDeviceState(selectedDeviceViewModel.selectedDeviceLiveData.value!!, which)
    }
  ...
  private fun updateDeviceInfo(deviceState: DeviceState?) {
    ...
    deviceUiModel?.let {
      ...
      val shapeDrawable = if (isOnline && mode != 0) shapeOnDrawable else shapeOffDrawable
      ...
      if (ON_OFF_SWITCH_DISABLED_WHEN_DEVICE_OFFLINE) {
        binding.onoffOption.isEnabled = isOnline
      } else {
        binding.onoffOption.isEnabled = true
      }
      binding.onoffOption.clearChecked()
      if(deviceUiModel.mode == 0L) {
        binding.onoffOption.check(binding.buttonOff.id)
      }
      if(deviceUiModel.mode == 1L) {
        binding.onoffOption.check(binding.buttonFree.id)
      }
      if(deviceUiModel.mode == 2L) {
        binding.onoffOption.check(binding.buttonBusy.id)
      }
