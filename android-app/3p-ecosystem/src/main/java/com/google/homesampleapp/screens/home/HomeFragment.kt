...
class HomeFragment : Fragment() {
  ...
  private val adapter =
      DevicesAdapter(
          ...
          { view, deviceUiModel, which ->

            Timber.d("onOff switch onClickListener: view [$view]")
            val onOffSwitch = view.findViewById<MaterialButtonToggleGroup>(R.id.onoff_option)
            Timber.d("onOff switch state: [${which}] [${onOffSwitch?.checkedButtonId}]")
            viewModel.updateDeviceStateOn(deviceUiModel, which.toLong())
          })
