...
class DevicesStateRepository @Inject constructor(@ApplicationContext context: Context) {
  ...
  suspend fun addDeviceState(deviceId: Long, isOnline: Boolean, mode : Long) {
    val newDeviceState =
  ...
  suspend fun updateDeviceState(deviceId: Long, isOnline: Boolean, mode: Long) {
    val newDeviceState =
        DeviceState.newBuilder()
            .setDeviceId(deviceId)
            .setDateCaptured(getTimestampForNow())
    ...
    if (!updateDone) {
      ...
      addDeviceState(deviceId, isOnline = isOnline, mode = mode)
    }
  }
