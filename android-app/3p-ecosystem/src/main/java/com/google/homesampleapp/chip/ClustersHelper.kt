...
import chip.devicecontroller.ChipClusters.LevelControlCluster.OnLevelAttributeCallback
...
class ClustersHelper @Inject constructor(private val chipClient: ChipClient) {
  ...
  // -----------------------------------------------------------------------------------------------
  // LevelCluster functions


  suspend fun setLevelDeviceStateOnOffCluster(deviceId: Long, level: Int, endpoint: Int) { //TODO add newcluster
    Timber.d(
        "setLevelDeviceStateOnOffCluster() [${deviceId}] level [${level}] endpoint [${endpoint}]")
    val connectedDevicePtr =
        try {
          chipClient.getConnectedDevicePointer(deviceId)
        } catch (e: IllegalStateException) {
      ...
      return suspendCoroutine { continuation ->  getLevelClusterForDevice(connectedDevicePtr, endpoint).moveToLevelWithOnOff(object : ChipClusters.DefaultClusterCallback {
          override fun onSuccess() {
              Timber.d("Success for setLevelDeviceStateOnOffCluster")
              continuation.resume(Unit)
          }

          override fun onError(ex: Exception) {
              Timber.e(ex, "Failure for setLevelDeviceStateOnOffCluster")
              continuation.resumeWithException(ex)
          }
  ...
  suspend fun getDeviceStateLevelCluster(deviceId: Long, endpoint: Int): Int? {
    ...
    return suspendCoroutine { continuation ->
      getLevelClusterForDevice(connectedDevicePtr, endpoint)
          .readCurrentLevelAttribute(
              object : CurrentLevelAttributeCallback {
                  override fun onSuccess(value: Int?) {
                      Timber.d("readCurrentLevelAttribute success: [$value]")
                      continuation.resume(value)
                  }

                  override fun onError(ex: Exception) {
                      Timber.e(ex, "readCurrentLevelAttribute command failure")
                      continuation.resumeWithException(ex)
                }
              })
    }
  ...
  private fun getLevelClusterForDevice(devicePtr: Long, endpoint: Int): ChipClusters.LevelControlCluster {
