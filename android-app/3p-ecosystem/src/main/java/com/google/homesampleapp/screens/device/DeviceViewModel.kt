...
constructor(
  ...
  fun updateDeviceState(deviceUiModel: DeviceUiModel, whichButton: Int) {
    Timber.d("updateDeviceState: which: [${whichButton}]")
    ...
    viewModelScope.launch {
      ...
      try {
        //TODO checked button stuff
        clustersHelper.setLevelDeviceStateOnOffCluster(deviceUiModel.device.deviceId, whichButton, 1)
        devicesStateRepository.updateDeviceState(deviceUiModel.device.deviceId, true, whichButton.toLong())
      } catch (e: Throwable) {
          ...
          override fun onReport(nodeState: NodeState) {
            ...
            viewModelScope.launch {
              devicesStateRepository.updateDeviceState(
                  deviceUiModel.device.deviceId, isOnline = true, mode.toLong())
            }
          }
        }
    viewModelScope.launch {
  ...
  private fun runDevicePeriodicUpdate(deviceUiModel: DeviceUiModel) {
    ...
    viewModelScope.launch {
      while (devicePeriodicPingEnabled) {
        ...
        mode = clustersHelper.getDeviceStateLevelCluster(deviceUiModel.device.deviceId, 1)
        if (mode == null) {
          Timber.e("[device ping] failed")
          mode = 0
          isOnline = false
        } else {
          Timber.d("[device ping] success [${mode}]")
          isOnline = true
        }
        devicesStateRepository.updateDeviceState(
            deviceUiModel.device.deviceId, isOnline = isOnline, mode.toLong())
